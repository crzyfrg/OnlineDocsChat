...
const INITIAL_URL_GROUPS: URLGroup[] = [
  { id: 'gemini-overview', name: 'Gemini Docs Overview', urls: GEMINI_DOCS_URLS, isEditable: false },
  { id: 'model-capabilities', name: 'Model Capabilities', urls: MODEL_CAPABILITIES_URLS, isEditable: false },
];

const LOCAL_STORAGE_KEY = 'documentation-browser-url-groups';
...
const App: React.FC = () => {
  const [urlGroups, setUrlGroups] = useState<URLGroup[]>(() => {
    try {
      ...
      console.error("Could not load URL groups from localStorage", error);
    }
    return INITIAL_URL_GROUPS;
  });

  const [activeUrlGroupId, setActiveUrlGroupId] = useState<string>(urlGroups[0]?.id || '');
  ...
  useEffect(() => {
    try {
      localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(urlGroups));
    } catch (error) {
      console.error("Could not save URL groups to localStorage", error);
    }
  }, [urlGroups]);
  ...
  const handleAddUrlGroup = (name: string) => {
    if (!name.trim()) return;
  ...
  const handleRemoveUrlGroup = (groupId: string) => {
      ...
      <div className="flex h-full w-full md:p-4 md:gap-4">
        ...
        `}>
          <KnowledgeBaseManager
            ...
            onRenameUrlGroup={handleRenameUrlGroup}
          />
        </div>
